...
const PROJECTS = [
  ...
  },
  {
    title: "Nudge",
    description:
      "A simple tool that lives on your status bar and lets send quick messages to your friends.",
  },
] as const

const SIGNATURE_LETTERS = ["s", "h", "u", "b", "h"]

export default function Page() {
  let staggerIndex = 0
  return (
    <main className="grid h-svh w-full grid-cols-5 grid-rows-5 gap-0 overflow-hidden bg-[#0a0a0a] font-mono">
      {/* Row 1: empty */}
      ...
      {/* Row 5: large letters s h u b h */}
      {SIGNATURE_LETTERS.map((letter, i) => (
        <GridCard key={`letter-${i}`} alignBottom staggerIndex={staggerIndex++}>
          <span
            className="text-[clamp(3rem,12vw,8rem)] leading-[0.85] font-bold tracking-tight text-foreground"
            aria-hidden
          >
            {letter}
          </span>
